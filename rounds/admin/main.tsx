...
import type { RevisionCard, Subject } from '../src/domain/content';
...
function subjectId(name: string, subjects: Subject[]) {
  const base = name
    .normalize('NFKD')
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '') || 'subject';
  if (!subjects.some((subject) => subject.id === base)) return base;
...
function App() {
  ...
  const cards = useMemo(() => {
    ...
    return library.workspace.catalog.cards.filter((card) => {
      const subject = library.workspace.catalog.subjects.find((item) => item.id === card.subjectId)?.name ?? card.subjectId;
      return [subject, card.title, card.prompt, card.facts.join(' '), card.pearl]
        .join(' ')
        .toLowerCase()
        .includes(q);
    });
  ...
  const save = async () => {
    if (!library) return;
    const values = [form.subject, form.title, form.question, form.management, form.pearl];
    ...
    setStatus('Saving…');
    try {
      const name = form.subject.trim();
      let subject = library.workspace.catalog.subjects.find((item) =>
        item.name.trim().toLowerCase() === name.toLowerCase() || item.id === name,
      );
      let workspaceForSave = library.workspace;
      if (!subject) {
        subject = {
          id: subjectId(name, library.workspace.catalog.subjects),
          name,
          symbol: '•',
          color: '#6F8870',
          topics: [],
        ...
        workspaceForSave = {
          ...library.workspace,
          catalog: {
            ...library.workspace.catalog,
            subjects: [...library.workspace.catalog.subjects, subject],
          },
        };
      }
      const existing = form.id
        ? workspaceForSave.catalog.cards.find((card) => card.id === form.id)
        : undefined;
      const base = existing ?? freshCard(subject.id);
      const input: RevisionCard = {
        ...base,
        subjectId: subject.id,
        topicId: `topic:${base.id}`,
